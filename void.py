...
with requests.Session() as s:
    ...
    if r.status_code == 200:
        ...
        r = s.post("https://scratch.mit.edu/site-api/comments/user/__V01D__/add/", headers=headers, data=json.dumps({"content": "void", "parent_id": "", "commentee_id": ""}))
        print(r.status_code)
        
        if r.text == "<script id=\"error-data\" type=\"application/json\">{\"error\": \"isDisallowed\"}</script>":
            print("Comments enabled.")
            s.post("https://scratch.mit.edu/site-api/comments/user/" + username + "/toggle-comments/", headers=headers)
        ...
        try:
            while True:
                ...
                if (len(comments) == 0):
                    wait = wait * 2
                for i in comments:
                    r = s.post("https://scratch.mit.edu/site-api/comments/user/" + username + "/del/", data=json.dumps({"id":i["data-comment-id"]}), headers=headers)
                    print(r.status_code)
